...
public class LaserRotateClass : MonoBehaviour, Interactable
{
    ...
    [SerializeField] private float rotateAngle = 45f;          // angele of rotation
    [SerializeField] private RotateDirection rotationDirection; //rotation selection (x/y)
    ...
    /// <summary>
    /// rotate the object on the selected axis with the selected angle
    /// </summary>
    public void Interact()
    {
        ...
        {
            //if the object has the X axis rotate it on the axis with the rotate amount angle 
            case RotateDirection.X:
                rotation = new Vector3(rotateAngle, 0, 0);
                break;
            //if the object has the Y axis rotate it on the axis with the rotate amount angle 
